...
/*
* size_t size : 総バイト数
* size_t stride : 1頂点あたりのバイト数
* const void* pInitData : 頂点データ
*/
VertexBuffer::VertexBuffer(size_t size, size_t stride, const void* pInitData)
{
    ...
    // 頂点バッファビューの設定
    m_View.BufferLocation = m_pBuffer->GetGPUVirtualAddress();  // 頂点バッファの仮想アドレス
    m_View.SizeInBytes = static_cast<UINT>(size);               // 総バイト数
    m_View.StrideInBytes = static_cast<UINT>(stride);           // 1頂点あたりのバイト数
