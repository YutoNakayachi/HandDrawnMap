...
ComPtr<ID3D12DescriptorHeap> Engine::CreateOneDescriptorHeapSRV()
{
...
bool Engine::Init(HWND hwnd, UINT windowWidth, UINT windowHeight)
{
    ...
    _gmemory = new DirectX::GraphicsMemory(Device()); 
    ...
    _spriteBatch = new DirectX::SpriteBatch(Device(), resUploadBatch, pd); 

    // SpriteFontオブジェクトの初期化
    _heapForSpriteFont = CreateOneDescriptorHeapSRV(); 
    ...
    WaitForCommandQueue(); 
    future.wait(); 
    _spriteBatch->SetViewport(GetViewPort()); 
...
void Engine::WaitForCommandQueue()
{
    auto currentIndex = g_Engine->CurrentBackBufferIndex();

    CommandQueue()->Signal(Fence(), m_fenceValue[currentIndex]); 

    if (Fence()->GetCompletedValue() < m_fenceValue[currentIndex])
    {
        auto event = CreateEvent(nullptr, false, false, nullptr); 

        Fence()->SetEventOnCompletion(m_fenceValue[currentIndex], event);
...
D3D12_VIEWPORT Engine::GetViewPort()const
{
    D3D12_VIEWPORT vp; 
